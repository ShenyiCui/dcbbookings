...
function generateEmptyPermaSched(min30){ //returns empty permasched
	var emptyPermaSched = [];
	var week1Data = []; 
	var week2Data = []; 
	//unbooked
	...
	//generating the array, WEEK 1
	for(var i = 0; i < 5; i++) // i is the number of cols
	{
		var dayData = [];
		for(var j = 0; j < dayLength; j++) // j is the number of rows
		{
			var periodData = [];
		...
		function validatePeriodAvailbility()//makes sure that the period isnt booked b4
		{
			...
			else
			{
				...
				if($("#startDate").val().length == 0){
					startingWeekBeginValidate = getWeekBegining(new Date(currentWeekBegining)); 
				}
				else{
				...
				for(var i = 0; i < RoombookingSchedule.length; i++)
				{
					currentWeekBeginValidate = (RoombookingSchedule[i][RoombookingSchedule[i].length-1])
					...
					if((RoombookingSchedule[i][4] == weekNow || RoombookingSchedule[i][4] == "Week1Week2") && compareArray(RoombookingSchedule[i][RoombookingSchedule[0].length-2],intendedCoors) && goAhead) // will only allow book to occur if the room hasnt been taken.
					{
...
function generateCalendar(bookingSched, permaSched, min30, calendarName, type, ownerEmail,currentWeekBeginning,currentWeek){
	if(type == "room"){
		...
		$.ajax({
			...
			url: "https://calendar.avenseguim.info/dcbbookings/api/createCalendar/submitData",
			type: "POST",
			data: JSON.stringify({
...
{
	document.getElementById("suscribeRoomCalendar").onclick = function(){openCalendarModal(resosID,resosType);}
...
{	
    ...
    function dataAnalysis()
    {
		...
		if(chosenResosJSON["Min30Periods"]=="false")//normal headers
		{
			tableColLength = 10;
			bookval = "booked";
			lockval = "locked";
    ...
    function mostFUser(resos)//returns array containing Most MostFreqUser and the number of times they visited.//top 10
    {
        ...
        if(schedule[0]!="Empty List")
        {
            ...
            for(var i =0; i<schedule.length; i++)//creating an array with users only appearing once and all users appearing. 
            {
				var schedToDate = transformYYYYMMDDtoDate(transformCurrentWeek(schedule[i][schedule[i].length-1]).toString())
			...
			for(var k =0; k < 10; k++)
			{
				...
				for(var i =0; i<allUsersOneTime.length; i++)
				{
					...
					{
						if(allUsersOneTime[i]==allUsersAllTime[j]) //seeing how many times they appear in the all time list compared to the one time list. 
						{
							counter +=1;
						}
					}
	...
	function popularWeekRoom(givenResos) // returns top 5 resources [[resource, numberOfBookings],[]] //global masteradmin
	{
